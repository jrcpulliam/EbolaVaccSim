...
punq$cols <- cols[as.character(punq[,lab])]pu
punq$pchs <- 21
...
plunq <- punq[threshold==.05 & lab %in% labstoshow, list(lab, power, trialStartDate, threshold, above, above_EV, caseSpent, totCase, totCase_EV,avHaz, tcal,date, cols, pchs)]
...
for(dd in tmpM[,unique(date)]) {
    ...
    xmax <- 60
    ## power
    with(tmp, plot(caseSpent, power, xlim = c(0,xmax), ylim = c(0,1), pch = pchs, las = 1, col = cols, cex = 2, bty = 'n', xlab = ''))
    mtext('avertable cases not averted', 1, -1, outer = T)
    ## speed
    with(tmp, plot(caseSpent, tcal/30, xlim = c(0,xmax), ylim = c(0,168/30), pch = pchs, las = 1, col = cols, cex=2, bty='n', xlab='', ylab = 'trial duration\n(months)'))
}
